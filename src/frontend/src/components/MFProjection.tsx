...
export default function MFProjection() {
  ...
  const { totalCurrentValue, totalInvested, fundGroups, chartData } = useMemo(() => {
    ...
    for (const h of mutualFunds) {
      ...
      if (existing) {
        ...
        map.set(h.schemeCode, { units: h.units, invested: h.units * h.purchaseNAV, currentNAV: h.currentNAV, name: h.schemeName });
      }
    }
  ...
  // ── Render ───────────────────────────────────────────────────────────────
  // No outer card wrapper — CollapsibleSection owns the chrome
  return (
      ...
      <div>
        ...
        <div className="rounded-lg border border-sidebar-border overflow-hidden">
          <table className="w-full text-xs">
            ...
            <tbody>
              {fundGroups.map((fund) => (
                <tr key={fund.schemeCode} className="border-b border-sidebar-border/50 last:border-0 hover:bg-accent/10 transition-colors">
